...
class CosineLRSchedule(UnicoreLRScheduler):
    ...
    def __init__(self, args, unicore_optimizer, total_train_steps):
        ...
        if isinstance(args.lr, Collection) and len(args.lr) > 1:
            raise ValueError(
                f"Cannot use a fixed learning rate schedule with cosine. Consider --lr-scheduler=fixed instead. ({args.lr})"
            )
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--min-lr", type=float, metavar="LR", help="min learning rate"
        )
        parser.add_argument(
            "--max-lr",
            type=float,
