...
try:
    ...
    def type_as(a, b):
        if paddle.is_tensor(x=a) and paddle.is_tensor(x=b):
            return a.to(b)
        else:
            return a
...
def safe_round(number, ndigits):
    ...
    elif torch is not None and paddle.is_tensor(x=number) and number.size == 1:
...
class MetersDict(OrderedDict):
    ...
    def load_state_dict(self, state_dict):
        ...
        for pri, key, meter_cls, meter_state in state_dict:
            ...
            meter.set_state_dict(state_dict=meter_state)
