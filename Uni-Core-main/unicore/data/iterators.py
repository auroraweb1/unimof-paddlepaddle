    ...
    def __iter__(self):
        for x in self.iterable:
            if self.n >= self.total:
                raise RuntimeError(
                    "Mismatch between actual and expected iterable length. This may be caused by resuming training from a checkpoint using a different number of GPUs, in which case you can try the --reset-dataloader option. Alternatively you may have a train or validation set that is smaller than the number of GPUs. If none of these apply, please report this to the unicore developers."
                )
            self.n += 1
            yield x
...
class EpochBatchIterator(EpochBatchIterating):
    ...
    def __init__(
        ...
        assert isinstance(dataset, paddle.io.Dataset)
    ...
    def first_batch(self):
        if len(self.frozen_batches) == 0:
            raise Exception(
                "The dataset is empty. This could indicate that all elements in the dataset have been skipped. Try increasing the max number of allowed tokens or using a larger dataset."
            )
    ...
    def load_state_dict(self, state_dict):
        ...
        if itr_pos > 0:
            if "len" in state_dict and state_dict["len"] != len(self):
                ...
                logger.info(
                    "Iterator size is changed. it is possible due to the change of update_freq/num_gpu. The itr_pos is change from {} to {} for consistency".format(
                        old_itr_pos, itr_pos
                    )
                )
            ...
            if self._next_epoch_itr is None:
                raise RuntimeError(
                    "Cannot resume training due to dataloader mismatch. You can relaunch training with `--reset-dataloader` and it should work."
                )
        else:
            self._next_epoch_itr = None
        ...
        if self._queue.qsize() < min(2, max(1, self._queue.maxsize // 2)):
            if time.time() - self.start_time > 5 * 60:
                if (
                    ...
                    logger.debug(
                        "Data loading buffer is empty or nearly empty. This may indicate a data loading bottleneck, and increasing the number of workers (--num-workers) may help."
                    )
