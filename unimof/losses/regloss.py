...
ATTR_REGESTRY = {
    ...
    "CarbonDioxide_298_0.01": [
        0.10396354700540493,
        0.22647223711266543,
        "standardization",
    ],
    "CarbonDioxide_298_0.05": [
        0.3552362138732747,
        0.4999100688274694,
        "standardization",
    ],
    "CarbonDioxide_298_0.1": [
        0.5899521612050606,
        0.6954814707840283,
        "standardization",
    ],
    "CarbonDioxide_298_0.5": [
        1.7881394809316975,
        1.4182319573028714,
        "standardization",
    ],
...
class Normalization(object):
    ...
    def transform(self, x):
        if self.normal_type == "log1p_standardization":
            return (paddle.log1p(x=x) - self.mean) / self.std
    ...
    def inverse_transform(self, x):
        if self.normal_type == "log1p_standardization":
            return paddle.expm1(x=x * self.std + self.mean)
        elif self.normal_type == "standardization":
...
class MOFV1MSELoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        sample_size = sample["target"]["finetune_target"].shape[0]
        _mean, _std, _normal_type = ATTR_REGESTRY[self.args.task_name]
        normalizer = Normalization(_mean, _std, _normal_type)
        loss = self.compute_loss(
            model, net_output[0], sample, normalizer, reduce=reduce
        )
        if not self.training:
            logging_output = {
                "loss": loss.data,
                "predict": normalizer.inverse_transform(
                    net_output[0].view(-1, self.args.num_classes).data
                ),
    ...
    def compute_loss(self, model, net_output, sample, normalizer, reduce=True):
        predicts = net_output.view(-1, self.args.num_classes).astype(dtype="float32")
        targets = (
            sample["target"]["finetune_target"]
            .view(-1, self.args.num_classes)
    ...
    def reduce_metrics(logging_outputs, split="valid") -> None:
        ...
        if "valid" in split or "test" in split:
            predicts = paddle.concat(
                x=[log.get("predict") for log in logging_outputs], axis=0
            )
            predicts = predicts.detach().cpu().numpy()
            targets = paddle.concat(
                x=[log.get("target") for log in logging_outputs], axis=0
            )
...
class MOFV2MSELoss(UnicoreLoss):
    ...
    def reduce_metrics(logging_outputs, split="valid") -> None:
        ...
        if "valid" in split or "test" in split:
            ...
            targets = targets.detach().cpu().numpy()
            task_name_list = [
                item for log in logging_outputs for item in log.get("task_name")
            ]
            ...
            if "test" in split:
                ...
                if df["task_name"].nunique() < 20 and df["task_name"].nunique() > 1:
                    stats = (
                        ...
                        .apply(lambda x: r2_score(x["targets"], x["predicts"]))
                        .to_dict()
                    )
                    for task_name, r2 in stats.items():
                        metrics.log_scalar(
                            "{}_r2_{}".format(split, task_name),
                            r2,
                            sample_size,
                            round=4,
                        )
